...
%         str.say('Written',b,'bytes of data to file:',newline,f,newline,'related to the periods of:',newline,c20.descriptor);
    ...
    function [mod,err]=common_ops(mode,v,product,mod,err)
      if iscellstr(mode)
        for i=1:numel(mode)
          if exist('err','var')
            [mod,err]=gravity.common_ops(mode{i},v,product,mod,err);
          else
            mod=gravity.common_ops(mode{i},v,product,mod);
          end
        end
      else
        %sanity
        ...
        case 'all'
          if exist('err','var')
            [mod,err]=gravity.common_ops(gravity.common_ops('mode_list'),v,product,mod,err);
          else
            mod=gravity.common_ops(gravity.common_ops('mode_list'),v,product,mod);
          end
        case 'consistent_GM'
