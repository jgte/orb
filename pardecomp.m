  ...
  methods(Static)
    ...
    function obj=join(pd_set,varargin)
      ...
      if file.exist(v.datafile) && ~v.force
        %load the mat data
        disp(['Loading join data from ',v.datafile])
        [obj,loaded_flag]=file.load_mat(v.datafile);
      ...
      if any(coeffidx==1)
        ...
        d=cell(1,obj.width);
        t=simpletimeseries.time2num(obj.t,pd_set.epoch,pd_set.timescale);
        T=pd_set.T;
        np=pd_set.np;
        t0=pd_set.t0;
        n=obj.width;
        ...
        s=time.progress(s,obj.width); clear s
        %TODO: the assignment below is very slow (~ 3min for degree 60), try to make it faster
        s.msg=['Propagating Parametric reconstruction of ',obj.descriptor]; s.n=obj.width;
        for i=1:obj.width
          obj=obj.set_cols(i,d{i});
          s=time.progress(s,i);
        end
      end
