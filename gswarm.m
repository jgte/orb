    ...
    function pod=plot_ops(obj,product,varargin)
      ...
      v=varargs.wrap('sources',{...
        ...
        {...
          ...
          'plot_save_data',      'no' ,@(i) islogical(i) || ischar(i);... %NOTICE: can be 'force' to skip reloading existing data
          'stats_relative_to', 'none' ,@(i) ischar(i);...
          'model_types',        {'*'} ,@(i) iscellstr(i);...
          'plot_lines_over_gaps_narrower_than',days(120),@isduration;...
          'plot_time_domain_source',0 ,@(i) isnumeric(i) && isscalar(i);...
          'force',              false ,@(i) islogical(i) || ischar(i);... %NOTICE: this turns plot_save_data to 'force' if true
        },...
        ...
        for qi=1:numel(v.plot_derived_quantity)
          ...
          if str.logical(v.plot_force) || ~file.exist(filenames{fn_idx})
            ...
            product.enforce_plot(v.varargin{:},...
              ...
              'plot_line_color_order',idx,...
              'plot_title',v.plot_title,...
              'plot_title_default',str.show({title_name,v.pod.title_wrt,title_suffix})...
            );
...
%       end
    ...
    function d=quality(varargin)
      ...
      %WORKFLOW: 
      % - you need to delete the last data file of:
      %   - swarm.sh.gswarm.rl01
      %   - swarm.sh.gswarm.rl01.err
      % - update the 'stop' input argument
      % - check the last entries in the time domain correspond to the most recent models
      %TODO: merge this with the validation method:
      % - augment the report there
      % - keep exporting the quality metric to the dissemination dir
      
      %parse inputs
      v=varargs.wrap('sources',{....
        {...
          'force_grace_pd',         false, @islogical;...
          'force',                  true, @islogical;...
      ...
      %fileame stem
      [~,f]=fileparts(strrep(ps.metadata.wildcarded_filename,'*_',''));
...
% 'swarm.sh.gswarm.rl01.tropical';... used in the rebuttal to reviewer 1
      ...
      c20opt='GSFC-7day';
%       c20opt='TN11'; %NOTICE: also uses climat. model based on GRACE+GRACE-FO data
      ...
      v=varargs.wrap('sources',{....
        {...
          ...
          'plot_fontsize_axis',  28      , @(i) isscalar(i) && isnumeric(i);...
          'plot_fontsize_title', 32      , @(i) isscalar(i) && isnumeric(i);...
          'plot_fontsize_label', 28      , @(i) isscalar(i) && isnumeric(i);...
          'plot_fontsize_legend',28      , @(i) isscalar(i) && isnumeric(i);...
      ...
      case 'dirs'
        d={...
          'metadata_dir',fullfile(v.work_dir,'metadata'),...
          'data_dir',    fullfile(v.work_dir,'data')...
        };
      case ''
...
%         version_list={'GSFC-7day','GSFC','TN-14','TN-11','TN-07'};
        ...
        saveas(gcf,fullfile(v.work_dir,'figures','C20-check.png'))
      case 'TN11' %not used anymore
        gravity.graceC20('mode','model-plot',v.varargin{:},'start',time.zero_date,'stop',datetime('2019-10-31'))
      ...
      case {'gracelowdegrees','gracefolowdegrees'} %fig 2
        ...
        d=datastorage(...
          'start',datetime('2002-04-01'),...
          'stop', datetime('2019-09-30'),...
          'debug',v.debug...
        ).init('grace.sh.rl06.csr',dirs{:}...
        ).init([type,'.sh.rl06.csr.pd.ts'],dirs{:}...
        );
        %make pretty plot
        plotting.figure(varargin{:});
        grs=d.data_get_scalar('grace_sh_rl06_csr/signal');
        grm=d.data_get_scalar([type,'_sh_rl06_csr_pd_ts' ]);
        for i=2:4
          plotting.figure
          grsp=grs.ts_C(i,0).add_expl_gaps(days(45)).plot('line',{'-o'});
          grmp=grm.ts_C(i,0).plot;
          plotting.enforce(v.varargin{:},...
            'plot_legend',{...
              str.rep(grsp.legend{1},['C',num2str(i),',0'],'CSR RL06');...
              str.rep(grmp.legend{1},['C',num2str(i),',0'],'12-parameter model');...
            },...
            'plot_title','none',...
            'plot_ylabel',['C_{',num2str(i),',0} [ ]']...
          );
          saveas(gcf,fullfile(v.work_dir,[type,'.C',num2str(i),'0.png']))
        end
      case 'pd' %this is used to recompute the pd models, remember to comment the never_force metadata in the products below
        dirs=gswarm.paper('type','dirs');
        d=datastorage(...
          'start',datetime('2002-04-01'),...
          'stop', v.stop,...
        ...
        pd_products={...
          'gracefo.sh.rl06.csr.pd','gracefo.sh.rl06.csr.pd.ts',...
            'grace.sh.rl06.csr.pd',  'grace.sh.rl06.csr.pd.ts'...
        };
        for i=1:numel(pd_products)
          d=d.init(pd_products{i},dirs{:},'force',true);
        end
      case 'pd-plot'
        de=[2,3];
        or=[0,0];
        ...
        for i=1:numel(dn_list)
          d=d.init(dn_list{i},dirs{:},...
            'force',v.force...
          );
          g{i}=d.data_get_scalar(dn_list{i});
        end
        for i=1:numel(de)
          plotting.figure;
          for gi=1:numel(g)
            g{gi}.plot('method','timeseries','degrees',de(i),'orders',or(i));
          end
          plotting.enforce(...
            'plot_legend',dn_list,...
            'plot_title',['C',num2str(de(i)),num2str(or(i))]...
          );
        end
      case 'smooth750'
        dirs=gswarm.paper('type','dirs');
        ...
        d=d.init('swarm.sh.gswarm.rl01.smooth750',dirs{:},...
          'force',v.force,...
          'plot_dir',v.work_dir,...
          'plot_time_domain_source',2,...
          v.subset('plot').varargin{:}...
        );
      case 'tropics'
        dirs=gswarm.paper('type','dirs');
        ...
        tropic_products={'swarm.sh.gswarm.rl01.tropical','swarm.sh.gswarm.rl01.nontropical'};
        for i=1:numel(tropic_products)
          d=d.init(tropic_products{i},dirs{:},...
            'force',v.force,...
            'plot_dir',v.work_dir,...
            v.subset('plot').varargin{:}...
          );
    ...
    function d=precombval(varargin)
      %special mode: define constants
      if nargin==1 && strcmp(varargin{1},'dirs')
        d={...
          'metadata_dir',file.unresolve_home('~/data/gswarm/analyses/metadata/'),...
...
%       ).init('gracefo.sh.rl06.csr.pd.ts',dirs{:});
      ...
      d=gswarm.production(...
        ...
        'stop',      dateshift(datetime('now'),'start','month'),...
        'inclusive', true,...     %this can be false, because the GRACE data is only used to derive gracefo.sh.rl06.csr.ld.ts, separately 
    ...
    function d=production(varargin)
      ...
      v=varargs.wrap('sources',{....
        {...
          'stop',      dateshift(datetime('now'),'end','month'),@isdatetime;...
      ...
      %update GRACE TN-11
      gravity.graceC20('mode','reload');
      %check if all needed arguments are available
      for i={'products','metadata_dir','data_dir','plot_dir'}
        assert(~isempty(v.(i{1})),['need argument ''',i{1},'''.'])
      end
      %create vector with relevant data product
      p=cellfun(@(i) dataproduct(i,...
        'plot_dir',v.plot_dir,...
        'metadata_dir',v.metadata_dir,...
        'data_dir',v.data_dir...
      ),v.products,'UniformOutput',false);
      %define data filename
      datafilename=fullfile(v.plot_dir,'d.mat');
