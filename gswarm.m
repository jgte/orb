      ...
      for i=1:v.pod.source.n
        ...
        for it=1:numel(v.plot_time)
          ...
          filename=file.build(v.pod.file_root,...
            ...
            strsplit(v.pod.source.names{i},' '),datestr(v.plot_time(it),'yyyymmdd'),'png'...
          );
          %plot only if not done yet
          if ~file.exist(filename,v.plot_force)
            if ~exist('grid_now','var')
              grid_now=v.pod.source.dat{i...
                ...
                ).grid('spatial_step',v.plot_spatial_step... %NOTICE: plot_spatial_step is also used in simplegrid.imagesc
              );
            end
...
%           'partitioning';...      %fig:res:comb:partitioning
    ...
    function get_input_data(mode)
      switch lower(mode)
      case('grace')
        %NOTICE: the download-l2.sh script iterates over specific years, automatically set from PROJECT 
        global PROJECT; %#ok<TLEV> 
        y=num2str(year(PROJECT.stop_date));
        disp('Downloading GRACE data')
        file.system(['./download-l2.sh CSR 06.1 ',y],'disp',true,'cd',grace.dir('base'));
        file.system(['./download-l2.sh CSR 06.2 ',y],'disp',true,'cd',grace.dir('base'));
        file.system(['./download-l2.sh CSR 06.3 ',y],'disp',true,'cd',grace.dir('base'));
        ...
        file.system('ln -sfv ../RL06.3/GSM-2_* .','disp',true,'cd',fullfile(grace.dir('l2'),'CSR','RL06'));
      case ('swarm')
        disp('Downloading Swarm data')
        file.system('~/bin/rsyncf.sh aristarchos --no-l2r','disp',true,'cd',gswarm.dir('data'));
      case ('kp')
        disp('Downloading Kp data')
        kp.download
      case('all')
    ...
    function out=c20model(mode,plot_dir,source)
      ...
      case 'clear'
        ...
        if file.exist(plotfile); delete(plotfile); end
        texfile=strrep(plotfile,'.png','.tex');
        if file.exist(texfile); delete(texfile); end
        out='';
      case 'done'
        plotfile=gswarm.c20model('filename',plot_dir);
    ...
    function [d,p]=precombval(varargin)
      ...
      [d,p]=gswarm.production(...
        'products',  {...
          'swarm.sh.gswarm.rl01.individual.land';...
          'swarm.sh.gswarm.rl01.individual';...
          'swarm.sh.gswarm.rl01.individual.maps';...
        },...
        'plot_pause_on_save',true,...
        'grace_model',false,.... %NOTICE: no longer showing statistics relative to GRACE model
        varargin{:}...
      );
    end
