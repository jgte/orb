            ...
            for di=1:numel(v.pod.mod.res)+1
              ...
              case numel(v.pod.mod.res)+1
                if v.plot_signal && ~str.none(product.mdget('stats_relative_to','default','none'))
                  tmp=v.pod.source.dat{v.pod.source.ref_idx}.interp(...
              ...
              %maybe detrend and remove outliers, depends on value of :
              % - v.plot_outlier_iter, handled inside the obj.outlier method 
              % - v.plot_detrended, handled inside the obj.detrend method (called by obj.outlier)
              tmp=tmp.outlier('detrend',v.plot_detrended,'outlier_iter',v.plot_outlier_iter);
              tmp=tmp.(v.plot_derived_quantity{qi});
              y(:,di)=tmp(:,end);
            end
            %average it
