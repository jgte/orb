...
% 'swarm.sh.gswarm.rl01.tropical';... used in the rebuttal to reviewer 1
      ...
      v=varargs.wrap('sources',{....
        {...
          ...
          'type',                   'all', @ischar;...
          'work_dir',fullfile(getenv('HOME'),'cloud','Work','articles','2019-04.gswarm'), @ischar;...
      ...
      case 'C20-sources'          %checking only
        plotfilename=fullfile(v.work_dir,'figures','checks','C20-sources.png');
        if ~file.exist(plotfilename)
%         version_list={'GSFC-7day','GSFC','TN-14','TN-11','TN-07'};
      ...
      case 'individual'           %fig:res:ind:cumdrms
        out=gswarm.paper(...
          'type','process',...
          'products',{['swarm.sh.gswarm.rl01.',v.type]}...
        );
      case {'smooth750'}          %checking only (used to confirm detrending is a good idea)
        out=gswarm.paper(...
          'type','process',...
          'start',datetime('2002-04-01'),...
          'products',{['swarm.sh.gswarm.rl01.',v.type]}...
      ...
      case 'tropics'
        plotfilename=fullfile(v.work_dir,'figures','checks','cumdas.ts.tropical.vs.non-tropical.png');
        if ~file.exist(plotfilename)
          functional='eqwh';
          out=gswarm.paper(...
            'type','process',...
            'products',{'swarm.sh.gswarm.rl01.tropical','swarm.sh.gswarm.rl01.nontropical'}...
          );
          t=out.data.swarm_sh_gswarm_rl01.signal.t_masked;
          str.say('gathering')
          d0={...
            out.data.grace_sh_rl06_csr_pd_ts;...
          ...
          di=cellfun(...
            ...
            ).interp(t,'interp_over_gaps_narrower_than',days(120)),...
          d0,'UniformOutput',false);
          str.say('Spatial pre-masking')
          gm={...
            di{1};...
          ...
          str.say('Computing residuals')
          grr=cellfun(@(i,j) i-j,gm,gr,'UniformOutput',false);
          swr=cellfun(@(i,j) i-j,gm,sw,'UniformOutput',false);
          str.say('Spatial post-masking')
          grr={...
            grr{1};grr{2};grr{3};...
            grr{1}.spatial_mask(    'tropical');...
            grr{1}.spatial_mask('non-tropical');...
          };
          swr={...
            swr{1};swr{2};swr{3};...
            swr{1}.spatial_mask(    'tropical');...
            swr{1}.spatial_mask('non-tropical');...
          };
          str.say('Detrending and computing DAS')
          grrd=cellfun(@(i) i.detrend.cumdas,grr,'UniformOutput',false);
          grrt=cellfun(@(i) i.detrend.t     ,grr,'UniformOutput',false);
          swrd=cellfun(@(i) i.detrend.cumdas,swr,'UniformOutput',false);
          swrt=cellfun(@(i) i.detrend.t     ,swr,'UniformOutput',false);
          
          str.say('Plotting pre vs post spatial masking')
          plotting.figure; hold on
          legend_str={};
          type_str={'global','pre tropical','pre non-tropical','post tropical','post non-tropical'};
          line_fmt={'mo','b+','rx','gd','cs'};
          m=grrd;t=grrt;mn='GRACE';n_fmt='-';
          for i=1:numel(m)
            plot(t{i},m{i}(:,end),[line_fmt{i},n_fmt])
            legend_str{end+1}=[mn,' ',type_str{i}]; %#ok<AGROW>
          end
          m=swrd;t=swrt;mn='Swarm';n_fmt='--';
          for i=1:numel(m)
            plot(t{i},m{i}(:,end),[line_fmt{i},n_fmt])
            legend_str{end+1}=[mn,' ',type_str{i}]; %#ok<AGROW>
          end
          plotting.enforce(...
            'plot_title_default','pre vs post spatial masking',...
          ...
          plotting.figure; hold on
          legend_str={};
          type_str={'global','tropical','non-tropical'};
          line_fmt={'go','b+','rx'};
          m=grrd;t=grrt;mn='GRACE';n_fmt='-';
          for i=1:3
            plot(t{i},m{i}(:,end),[line_fmt{i},n_fmt])
          ...
          plotting.enforce(...
            'plot_title_default','Tropical vs. High Lat. residual w.r.t. GRACE climat. model',...
            'plot_ylabel',gravity.functional_label(functional),...
            'plot_legend',legend_str...
          );
          plotting.save(plotfilename);
