classdef gswarm
  ...
  %           legend('GRACE',m.descriptor)
    ...
    function out=plot_ops(obj,product,varargin)
      ...
      otherwise
        for i=1:out.source.n
          str.say(out.source.datanames{i}.filename,'?=',v.stats_relative_to)
          if str.iseq(out.source.datanames{i}.filename,v.stats_relative_to)
      ...
      for i=1:out.source.n
        ...
        if ~file.exist(filename)
          ...
          plotting.enforce(v.varargin{:},...
            ...
            'plot_title',str.show({...
              'temporal STD of ',upper(out.source.names{i}),...
...
%       else
        %init user feedback
        s.msg='Loading/computing catchments';s.n=numel(v.catchment_list)*out.source.n;s.stationary=false;
        pd_args={...
          'T',time.duration2num(time.num2duration(v.sin_period,v.sin_period_unit),v.timescale),...
          'np',v.polyorder+1,...
          't0',time.duration2num(obj.start-v.t0,v.timescale)...
          'epoch',out.source.signal{1}.epoch,...
          'timescale',v.timescale,...
          'descriptor',out.source.signal{1}.descriptor,...
        };
...
%       end
      ...
      for j=1:numel(v.catchment_list)
        %build filename
        filename=file.build(out.file_root,...
          v.plot_functional,'catch',out.file_smooth,strsplit(v.catchment_list{j},' '),'tex'...
        );
        %plot only if not done yet
        if ~file.exist(filename)
      ...
      %build filename
      filename=file.build(out.file_root,'all_catchments.tex');
      ...
      obj=gswarm.plot_temporal_std(obj,product,varargin{:});
    ...
    function d=quality(varargin)
      ...
      f=fullfile(getenv('HOME'),'data','gswarm','dissemination',[f,'.',datestr(now,'yyyy-mm-dd'),'.quality']);
      ...
      gm=g.set_lmax(40).spatial_mask('deep ocean');
      str.say(' --- Smoothing, computing geoid and cumulative RMS:')
      [~,cts]=gm.scale(750e3,'gauss').scale('geoid','functional').cumdrms;
    ...
    function validation(varargin)
      v=varargs.wrap('sources',{....
        {...
          ...
          'inclusive', false,                     @islogival;... %if this is true, all GRACE period will be plotted
      ...
      %define data dir
      datadir=file.unresolve(['~/data/gswarm/analyses/',v.date,'/']);
      %create vector with relevant data product
      p=cellfun(@(i) dataproduct(i,'plot_dir',datadir),v.products,'UniformOutput',false);
      %stop date coincides with analysis date
      stop=datetime(v.date);
      %define data filename
      datafilename=fullfile(datadir,'d.mat');
      %load data if already available
      if file.exist(datafilename) && ~v.force
        str.say('Loading analysis data from',datafilename)
        load(datafilename,'d')
      else
        d=datastorage('debug',v.debug,'start',v.start,'stop',stop,'inclusive',v.inclusive);
...
%       datadir=file.unresolve('~/data/gswarm/analyses/2019-05-06/');
%       p=cellfun(@(i) dataproduct(i,'plot_dir',datadir),{...  
...
%       inclusive=true;
      ...
      datadir=file.unresolve('~/data/gswarm/analyses/2019-05-26/');
      p=cellfun(@(i) dataproduct(i,'plot_dir',datadir),{...  
